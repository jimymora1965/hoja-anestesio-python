...
def obtener_peso():
    while True:
        try:
            peso = float(input("Peso del paciente (en kilogramos): "))
            if 0 <= peso <= 200:
                return peso
            else:
                print("El peso debe estar en el rango de 0 a 200 kilogramos.")
        except ValueError:
            print("Por favor, ingrese un peso válido (número).")
...
def main():
    ...
    with open(nombre_archivo, "w") as archivo:
        ...
        archivo.write("Peso: " + str(hoja.peso) + "\n")
